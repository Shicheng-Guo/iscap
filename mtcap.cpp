...
unsigned	Capture::hits(char	*S){
	...
	for(unsigned	j=0;	j+hash_len<=len;	j++){
		uint64_t	k=t1ha(S+j,	hash_len);
		if(bloom[k&0x3fffff]&&bloom[rotl(k,22)&0x3fffff]&&bloom[rotl(k,44)&0x3fffff])	h++;
	}	
	return	h;
}
...
void	Capture::fasta2keys(void){
	...
	bloom.resize(1ULL<<22);	string	ns;	ns.assign(hash_len,	'N');	uint64_t	hashn=t1ha(ns.c_str(),	hash_len);
	for(size_t	i=0;	i<fasta.size();	i++){
		string	&s=fasta[i];
		for(size_t	j=0;	j<s.size();	j++)	s[j]=fwd[(unsigned char)s[j]];
		for(size_t	j=0;	j+hash_len<=s.size();	j++){
			uint64_t	k=t1ha(s.c_str()+j,	hash_len);	if(k==hashn)	continue;
			bloom[k&0x3fffff]=1;	bloom[rotl(k,22)&0x3fffff]=1;	bloom[rotl(k,44)&0x3fffff]=1;
		}	
...
void	document(void) {
	cerr<<"Usage:	mtcap [options] target output read1 read2\n";
...
int	main(int	ac,	char	**av){
	size_t	t0=time(NULL);
	cerr<<"\033[41m";	cerr<<"                          ";	cerr<<"\033[0m\n";
	cerr<<"\033[41m";	cerr<<"    \033[33m★                     ";	cerr<<"\033[0m\n";
	cerr<<"\033[41m";	cerr<<"  \033[33m★  ★                    ";	cerr<<"\033[0m\n";
	cerr<<"\033[41m";	cerr<<"     \033[33m★                    ";	cerr<<"\033[0m\n";
	cerr<<"\033[41m";	cerr<<"    \033[33m★                     ";	cerr<<"\033[0m\n";
	cerr<<"\033[41m";	cerr<<"                   \033[37mmtCap  ";	cerr<<"\033[0m\n";
	cerr<<"\033[41m";	cerr<<"                  \033[37mYi Wang ";	cerr<<"\033[0m\n";
	cerr<<"\033[41m";	cerr<<"                          ";	cerr<<"\033[0m\n\n";		
